-- 062_directory_local_operator.sql
-- An employer that only operates locally speaks for its own place.
--
-- WHY. Under 061 a company's own published policy ("confirmed_corporate") never
-- earns the mark alone, because a chain's national policy says nothing about
-- the store down the road. That rule also denied the mark to Flathead County,
-- whose own HR page says a conviction does not automatically disqualify an
-- applicant: for a county government, the policy IS the local employer. Troy,
-- 2026-09-24: "yes count as local. we are looking for creative solutions to
-- reentering citizens looking for work."
--
-- THE RULE, AS DATA. directory_claim_policy gains local_operator_earns_mark.
-- Only confirmed_corporate sets it. It applies when the employer operates only
-- locally: org_kind = 'government', or operating_model = 'independent' (a
-- single-site or locally-owned business, set by a person, never guessed).
-- Chains ('corporate', 'franchise', 'unknown') are unchanged: their policy
-- still reads "company policy, not checked here".
--
-- The standing view keeps its output columns, so every view built on it is
-- untouched. Expiry, write-once evidence, negatives and grants are unchanged.

ALTER TABLE directory_claim_policy ADD COLUMN IF NOT EXISTS local_operator_earns_mark BOOLEAN NOT NULL DEFAULT false;
UPDATE directory_claim_policy
   SET local_operator_earns_mark = true,
       meaning = 'An official company policy. For a chain it never earns the mark alone: a local yes is needed. For an employer that only operates locally (a government, or a business marked independent) its own policy is the local yes.'
 WHERE claim_type = 'confirmed_corporate';
