...
**Last session (Opus 4.8):** Full journey instrumentation DONE + verified -- Stages 3 (resume), 4 (disclosure), 5 (interview) all now persist real signals, so `computeNextStep` walks 3 -> 4 -> 5 -> 6 on live data (13/13 + 10/10 against the live DB). **Troy's privacy decision (2026-06-07):** keep data stored so the page is progressive and the tools work as designed, but revise the wording to be honest + reassuring + secure. For interview practice specifically: teach frames, not scripts -- store the FRAME practiced and whether the meaning landed (the coach's feedback), never the user's words/transcript/audio. The two stale "Nothing here is saved or shared" lines are replaced accordingly. See the "2026-06-07 (session 2)" section below.
**Next session:** Coach needs real browser QA. Stage 6 instrumentation (applications already write; verify follow_up_at fires the "time to follow up" step). Then preview deploy + QA checkpoint (push Tier-0 keys to Vercel -> verify `/forgot-password` email -> deploy preview). The revised privacy copy + the site privacy policy should get an attorney pass before public launch. All work remains local commits -- do NOT push/deploy until the password-reset email is verified with the new Resend key. Twilio A2P pending (non-blocking).
