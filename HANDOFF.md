...
**Next session:** Run the preview gates (Troy is setting Vercel env + will test): `/forgot-password` email via new Resend key -> sign-in -> journey 3->4->5->6 on the preview. Seed the new Neon's access codes + admin/Baker (it's empty). Then merge `session/journey-instrumentation-2026-06-07` -> `main` (= prod deploy, git-connected). Coach still needs real browser QA. The revised privacy copy + site privacy policy should get an attorney pass before public launch. Remaining feature work: W5 vault/materials, W6 Twilio (A2P pending), W7 partner dashboard (Baker priority), W9 admin/health, W10 conference/OSS.
